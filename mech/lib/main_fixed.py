# WebSocket Mech Controller - Fixed Version
# Real-time servo updates without blocking
# All sensors included: Gas, Servos, Temperature, Ultrasonic
...
class WebSocketClient:
    ...
    def connect(self):
        ...
        # Set non-blocking and create poller
    ...
    def recv(self):
        ...
        events = self.poller.poll(0)  # Non-blocking check
        if not events:
            return None
...
class MechController:
    def __init__(self):
        print("\n" + "=" * 70)
        print("  MECH CONTROLLER - Fixed Non-Blocking Version")
        print("=" * 70 + "\n")
        ...
        try:
            ...
            if msg_type == 'control_servo':
                ...
                if self.emergency_lock and not emergency_override:
                    print("⚠ Servo control blocked - Emergency active")
                    return
                ...
                if servo and angle is not None and self.servos:
                    ...
                    print(f"✓ {servo} → {angle}°{override_msg}")
                ...
                if servos and self.servos:
                    ...
                    override_msg = " [OVERRIDE]" if emergency_override else ""
                    print(f"✓ Servos updated{override_msg}")
    ...
    def run(self):
        """Main WebSocket loop with proper non-blocking I/O"""
        # Connect to WiFi
        try:
        ...
        while True:
            try:
                ...
                print("=" * 70)
                print("  RUNNING - Real-time servo updates enabled")
                print("=" * 70 + "\n")
                ...
                while True:
                    ...
                    # Check for incoming messages (non-blocking with poll)
                    msg = ws.recv()
                    if msg:
                        message_count += 1
                        print(f"← [{message_count}] Message received")
                        self.handle_message(msg)
                    
                    # Very small delay - allows fast servo updates
                    time.sleep(0.005)  # 5ms = 200Hz update rate
